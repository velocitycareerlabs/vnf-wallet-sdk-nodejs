import { VCLDeepLink, VCLJwt, VCLPresentationRequest, VCLVerifiedProfile } from "../../../../src";
import { DidJwkMocks } from "./DidJwkMocks";

export class PresentationRequestMocks {
    static readonly EncodedPresentationRequest =
        "eyJ0eXAiOiJKV1QiLCJraWQiOiJkaWQ6dmVsb2NpdHk6MHhkNGRmMjk3MjZkNTAwZjliODViYzZjN2YxYjNjMDIxZjE2MzA1NjkyI2tleS0xIiwiYWxnIjoiRVMyNTZLIn0..BSQnMNNJyicCsh6zeh7k5GBHC6T9QgPNV4SHhSXsnz3sBJMwNBFz7v4axLCoCiKHtIxNj5-bJ5ggI3wF6UVDeQ"

    static readonly EncodedPresentationRequestResponse =
        `{"presentation_request":"${PresentationRequestMocks.EncodedPresentationRequest}"}`

    static readonly PresentationRequestJwt = VCLJwt.fromEncodedJwt(PresentationRequestMocks.EncodedPresentationRequest)

    static readonly JWK =
        "{\"alg\":\"ES256K\",\"use\":\"sig\",\"kid\":\"uemn6l5ro6hLNrgiPRl1Dy51V9whez4tu4hlwsNOTVk\",\"crv\":\"secp256k1\",\"x\":\"oLYCa-AlnVpW8Rq9iST_1eY_XoyvGRry7y1xS4vU4qo\",\"y\":\"PUMAsawZ24WaSnRIdDb_wNbShAvfsGF71ke1DcJGxlM\",\"kty\":\"EC\"}\n"

    static readonly PresentationRequest = new VCLPresentationRequest(
        PresentationRequestMocks.PresentationRequestJwt,
        new VCLVerifiedProfile({}),
        new VCLDeepLink(""),
        null,
        DidJwkMocks.DidJwk,
    )
}